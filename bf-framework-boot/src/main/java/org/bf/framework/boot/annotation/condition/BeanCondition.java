 ...
 */
@Slf4j
class BeanCondition implements Condition,ConfigurationCondition {
...
//		return ConfigurationPhase.PARSE_CONFIGURATION;
	...
	public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
		...
		if (annotations.isPresent(OnBean.class)) {
			Spec<OnBean> spec = new Spec<>(context, metadata, annotations, OnBean.class);
		...
		if (metadata.isAnnotated(OnMissingBean.class.getName())) {
			Spec<OnMissingBean> spec = new Spec<>(context, metadata, annotations,
					OnMissingBean.class);
			MatchResult matchResult = getMatchingBeans(context, spec);
	...
	private Set<String> collectBeanNamesForType(ListableBeanFactory beanFactory, boolean considerHierarchy,
		...
		if (considerHierarchy && beanFactory instanceof HierarchicalBeanFactory) {
			...
			if (parent instanceof ListableBeanFactory) {
				result = collectBeanNamesForType((ListableBeanFactory)parent, considerHierarchy, type, parameterizedContainers,
						result);
			}
		}
		return result;
...
//	}
	...
	private BeanDefinition findBeanDefinition(ConfigurableListableBeanFactory beanFactory, String beanName,
		...
		if (considerHierarchy
				&& beanFactory.getParentBeanFactory() instanceof ConfigurableListableBeanFactory) {
